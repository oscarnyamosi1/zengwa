...
// Define a type for gallery items (adjust if you have a shared type)
type GalleryItem = {
  id: string;
  type: 'photo' | 'video';
  image: string;
...
// Helper to convert a video URL to an embed URL with autoplay
const getEmbedUrlWithAutoplay = (url: string): string => {
  try {
    const u = new URL(url);
    // YouTube
  ...
  // If it's already an embed URL, try to add autoplay
          ...
          <div className="relative max-w-4xl w-full" onClick={(e) => e.stopPropagation()}>
            ...
            {lightboxItem.type === 'video' ? (
              <div className="relative aspect-video w-full bg-black rounded-2xl overflow-hidden">
                <iframe
                  src={getEmbedUrlWithAutoplay(lightboxItem.image)}
